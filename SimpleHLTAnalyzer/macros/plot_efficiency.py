...
def fillHistograms(probe, tag,  parameter, tree):
	...
	# cheap trick to get the second max if it exists, max ow
        tree.Draw("(MaxIf$(%s,%s<Max$(%s)))>>%s"               % (parameter, parameter, parameter,  numerator.GetName()),    "(%s == 1) && (%s == 1)" % (tag, probe))
        # debugging ------------------------
        ...
        # ----------------------------------
        tree.Draw("(MaxIf$(%s,%s<Max$(%s)))>>%s"               % (parameter, parameter, parameter,  denominator.GetName()), "(%s == 1)"              % (tag       ))
        # debugging ------------------------
...
for tag in tags:
        makeCorrelationPlots(tag, "bTagCSVOnline", "bTagCSVOffline", "1")
